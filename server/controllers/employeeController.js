...
const addEmployee = async (req, res) => {
    try { 
        ...
        const newEmployee = new Employee({
            ...
            department,
            salary,
            isDepartmentHead: role === "departmentHead" ? true : false,
        })
...
const updateEmployee = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(404).json({success :false , error :"Uesr not found"})
        }
        ...
        return res.status(200).json({success : true , messege :"employee update"})
